...
public class ChatRepository {
    ...
    public void markBuyerRead(@NonNull String conversationId) {
        Map<String, Object> data = new HashMap<>();
        data.put("buyerUnread", false);
        firestore.collection("conversations").document(conversationId)
                .set(data, SetOptions.merge());
    }
    ...
    private void updateLastMessage(@NonNull DocumentReference convRef,
        ...
        data.put("lastMessageAt", FieldValue.serverTimestamp());
        data.put("updatedAt", FieldValue.serverTimestamp());
        // Buyer message → staff badge; staff reply → buyer badge (no inbox notification).
        if (ChatMessage.SENDER_USER.equals(senderType)) {
            data.put("staffUnread", true);
        } else if (ChatMessage.SENDER_SELLER.equals(senderType)) {
            data.put("staffUnread", false);
            data.put("buyerUnread", true);
        }
    ...
    private Conversation mapConversation(@NonNull DocumentSnapshot doc) {
        ...
        Boolean buyerUnread = doc.getBoolean("buyerUnread");
        c.setBuyerUnread(buyerUnread != null && buyerUnread);
