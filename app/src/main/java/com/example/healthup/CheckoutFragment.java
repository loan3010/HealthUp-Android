...
public class CheckoutFragment extends Fragment {
    ...
    private void continuePlaceOrder(@NonNull FirebaseUser user) {
        ...
        db.collection("users").document(userId).collection("cart").get().addOnSuccessListener(cartSnapshot -> {
            if (!isAdded()) return;
            WriteBatch batch = db.batch(); batch.set(orderRef, order);
            DocumentReference notifRef = db.collection("users").document(userId).collection("notifications").document();
            Map<String, Object> notif = new HashMap<>(); notif.put("type", "ORDER_UPDATE");
            if ("en".equals(currentLang)) { notif.put("title", "Order successful"); notif.put("body", "Order #" + order.getOrderCode() + " placed successfully. Total: " + formatVnd(finalAmount)); }
            else { notif.put("title", "Đặt hàng thành công"); notif.put("body", "Đơn hàng #" + order.getOrderCode() + " đã đặt thành công. Tổng tiền: " + formatVnd(finalAmount)); }
            notif.put("message", notif.get("body")); notif.put("orderId", orderRef.getId()); notif.put("createdAt", FieldValue.serverTimestamp()); notif.put("read", false);
            batch.set(notifRef, notif);

            for (CartItem ci : selectedItems) { DocumentReference cartDocRef = resolveCartDocument(cartSnapshot, ci); if (cartDocRef != null) batch.delete(cartDocRef); }
            batch.commit().addOnSuccessListener(aVoid -> { if (isAdded()) showSuccessDialog(); }).addOnFailureListener(e -> { if (isAdded()) { btnPlaceOrder.setEnabled(true); btnPlaceOrder.setText("Đặt hàng"); Toast.makeText(getContext(), "Lỗi đặt hàng", Toast.LENGTH_SHORT).show(); } });
        });
    }
